	...
	for (size_t i = 0; i < equations.size(); i++) {
		...
		default:
			if (eq.sign1 == eq.sign2 * eq.operation) {
				//common addition
				results.insert(results.end(), addition(eq.number1, eq.number2, eq.sign1, false));
				break;
			} 
...
Result Calculator::addition(vector<int> number1, vector<int> number2, int resultSign, bool subtraction) {
	Result result;
...
Result Calculator::multiplication(vector<int> number1, vector<int> number2, int resultSign) {
	Result result;
	...
	for (size_t i = 1; i < partials.size(); i++) { //Sum of every partials to get the multiplication final result
		aux = addition(aux, partials[i], 1, false).number;
	}
